...
namespace  {
	...
	void to_json_recursively(const rttr::instance& obj2, nlohmann::json& writer) {
		...
		for (auto prop : prop_list) {
			...
			//	continue;
			const auto meta = prop.get_metadata(MetaInfo::FLAGS);
			if (meta) {
				const auto flags = meta.get_value<MetaInfo::Flags>();
				if ((flags & MetaInfo::Flags::SERIALIZABLE) == 0) {
					continue;
				}
			}
...
Result<JsonOk, JsonError> IKIGAI::UTILS::fromJsonRecursively(rttr::instance obj2, nlohmann::json& json_object) {
	rttr::instance obj = obj2.get_type().get_raw_type().is_wrapper() ? obj2.get_wrapped_instance() : obj2;
	...
	for (auto prop : prop_list) {
		...
		if (ret == json_object.end()) {
			if (auto defVal = prop.get_metadata(MetaInfo::DEFAULT)) {
				prop.set_value(obj, defVal);
				continue;
			}
			...
			if (meta) {
				const auto flags = meta.get_value<MetaInfo::Flags>();
				if (flags & (MetaInfo::Flags::OPTIONAL_PARAM)) {
					continue;
				}
				if ((flags & MetaInfo::Flags::SERIALIZABLE) == 0) {
					continue;
			...
			LOG_ERROR("Not found: " + prop.get_name().to_string());
		}
...
std::string IKIGAI::UTILS::ToJsonStr(rttr::instance obj) {
	if (!obj.is_valid()) {
		return std::string();
	}
...
nlohmann::json  IKIGAI::UTILS::ToJson(rttr::instance obj) {
	if (!obj.is_valid()) {
		return std::string();
	}
