...
void TextEditor::SetTextLogLines(const std::list<Record>& aLines, const std::map<IKIGAI::UTILS::LOGG::Log::Level, bool> logLevels, const std::string& search)
{
	mLines.clear();
	...
	if (aLines.empty()) {
		...
		for (auto& record : aLines) {
			if (logLevels.at(record.mLogLevel) &&
				(search.empty() ||
				record.mMsg.find(search) != record.mMsg.npos)) {
				mLines.push_back(Line());
				mLines.back().reserve(record.mMsg.size());
				for (size_t j = 0; j < record.mMsg.size(); ++j) {
					mLines.back().emplace_back(Glyph(record.mMsg[j], PaletteIndex::Default));
				}
			}
		}
	}
...
const TextEditor::Palette& TextEditor::GetLoggerPalette()
{
	const static Palette p = { {
			IM_COL32(230, 230, 230, 255),	// None
			IM_COL32(244, 47, 47, 255),	// Keyword ERROR
			IM_COL32(47, 169, 245, 255),	// Number DEBUG
			IM_COL32(47, 245, 151, 255),	// String INFO
			IM_COL32(230, 230, 230, 255), // Char literal
			0xffffffff, // Punctuation
			0xff008000,	// Preprocessor
			IM_COL32(245, 196, 47, 255), // Identifier WARNING
			0xff606010, // Known identifier
			0xffc040a0, // Preproc identifier
			0xff206020, // Comment (single line)
			0xff406020, // Comment (multi line)
...
static bool TokenizeNamedString(const char* in_begin, const char* in_end, const char*& out_begin, const char*& out_end, const std::string& word)
{
	const char* p = in_begin;
...
const TextEditor::LanguageDefinition& TextEditor::LanguageDefinition::Logger()
{
	static bool inited = false;
	static LanguageDefinition langDef;
	if (!inited) {
		...
		langDef.mTokenize = [](const char* in_begin, const char* in_end, const char*& out_begin, const char*& out_end, PaletteIndex& paletteIndex) -> bool {
			...
			else if (TokenizeNamedString(in_begin, in_end, out_begin, out_end, "INFO:"))
				paletteIndex = PaletteIndex::String;
			else if (TokenizeNamedString(in_begin, in_end, out_begin, out_end, "DEBUG:"))
				paletteIndex = PaletteIndex::Number;
			else if (TokenizeNamedString(in_begin, in_end, out_begin, out_end, "ERROR:"))
				paletteIndex = PaletteIndex::Keyword;
			else if (TokenizeNamedString(in_begin, in_end, out_begin, out_end, "WARNING:"))
				paletteIndex = PaletteIndex::Identifier;
			//else if (TokenizeCStyleIdentifier(in_begin, in_end, out_begin, out_end))
			//	paletteIndex = PaletteIndex::Identifier;
			//else if (TokenizeCStyleNumber(in_begin, in_end, out_begin, out_end))
