...
namespace ImGui {
    ...
    IMGUI_API bool BeginNeoTimelineEx(const char* label, bool * open = nullptr, ImGuiNeoTimelineFlags flags = ImGuiNeoTimelineFlags_None, bool* buttonPressed = nullptr, bool* isSelected = nullptr);
    IMGUI_API ImVec2 GetAddFrame();
    IMGUI_API int NeoKeyframe(int32_t* value); //1 need delete 2 select

    IMGUI_API bool IsNeoKeyframeHovered();
    IMGUI_API bool IsNeoKeyframeSelected();
    IMGUI_API bool IsNeoKeyframeRightClicked();

    IMGUI_API int GetSelectedTimelineId();
    // Selection API
