...
namespace ImGui
{
    ...
    static int getMosePosFrame(ImGuiNeoSequencerInternalData& context) {
        const auto& imStyle = GetStyle();
    ...
    static int createKeyframe(int32_t* frame)
    {
        const auto& imStyle = GetStyle();
...
//
                      ...
                      ImGuiNeoSequencerFlags flags)
    {
        ...
        context.currentTimelineId = -1;
        context.currentAddFrameId = -1;
        context.selectedLineId = -1;
    ...
    int GetSelectedTimelineId() {
        auto& context = sequencerData[currentSequencer];
        return context.selectedLineId;
    }
    ...
    bool BeginNeoTimelineEx(const char* label, bool* open, ImGuiNeoTimelineFlags flags, bool* buttonPressed, bool* isSelected)
    {
        IM_ASSERT(inSequencer && "Not in active sequencer!");
        ...
        if (!isGroup && IsMouseHoveringRect(area.Min, area.Max)) {
            ...
            if (IsMouseDoubleClicked(ImGuiMouseButton_Left)) {
                context.currentAddFrameId = getMosePosFrame(context);
            }
        }
        ...
        if (id == context.SelectedTimeline) {
            context.selectedLineId = context.currentTimelineId;
            if (isSelected) {
                *isSelected = true;
            }
        }
    ...
    {
        auto& context = sequencerData[currentSequencer];
        return {static_cast<float>(context.currentTimelineId), static_cast<float>(context.currentAddFrameId)};
    }
    ...
    int NeoKeyframe(int32_t* value)
    {
        IM_ASSERT(inSequencer && "Not in active sequencer!");
        auto& context = sequencerData[currentSequencer];
        IM_ASSERT(!context.TimelineStack.empty() && "Not in timeline!");

        return createKeyframe(value);
    }
