...
namespace std {
	...
	template <typename T> struct hash<IKIGAI::object_ptr<T>> {
		constexpr size_t operator()(IKIGAI::object_ptr<T> const& p) const
			noexcept {
			return hash<T*>()(p.get());
		}
	};
	...
	typename std::enable_if<
		...
		IKIGAI::object_ptr<To>>::type
		static_pointer_cast(IKIGAI::object_ptr<From> p) {
	...
	typename std::enable_if<
		...
		IKIGAI::object_ptr<To>>::type
		dynamic_pointer_cast(IKIGAI::object_ptr<From> p) {
