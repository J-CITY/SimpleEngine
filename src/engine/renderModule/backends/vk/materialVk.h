...
namespace IKIGAI::RENDER {
	...
	public:
		void fillUniformsWithShader(std::shared_ptr<ShaderVk> shader, std::shared_ptr<TextureInterface> defaultTexture, bool useTextures);


		std::string mPath;

		void setShader(std::shared_ptr<ShaderVk> shader);
		...
		void setIsDeferred(bool v, std::shared_ptr<ShaderInterface> shader) override {
			...
			if (mIsDeferred) {
				setShader(std::dynamic_pointer_cast<ShaderVk>(shader));
			}
			else {
				mUniforms.clear();
				mUniformData.clear();
				mShader = std::dynamic_pointer_cast<ShaderVk>(shader);
			}
		}
		...
		void prepareTextureUniforms(std::shared_ptr<TextureVk> emptyTexture);
	};
}
#endif
