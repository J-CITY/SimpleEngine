...
layout(binding = 6) uniform sampler2D engine_dirShadowMap;
layout(binding = 7) uniform sampler2DArray engine_dirBakedShadowMap;
        ...
        float     zReceiver) 
{
    ...
    for(int i = 0; i < engine_ShadowUBO.dirBLOCKER_SEARCH_NUM_SAMPLES; i++) {
        ...
        float sampleDepth = texture(engine_dirShadowMap, uv + offset *  searchWidth).r;
      ...
      float filter_radius) 
{
    ...
    for(int i = 0; i < engine_ShadowUBO.dirPCF_NUM_SAMPLES; i++) {
        ...
        float sampleDepth = texture(engine_dirShadowMap, uv + offset).r;
        if(sampleDepth < (zReceiver - bias)) {
            sum += 1.0;
        }
    }
...
float DirShadowCalculation(vec4 fragPosLightSpace)
{
    ...
    float closestDepth = texture(engine_dirShadowMap, projCoords.xy).r; 
    ...
    vec3 normal = normalize(g_Normal);
    vec3 lightDir = normalize(engine_ShadowUBO.dirLightPos - g_FragPos);
    ...
    // PCF
    //float shadow = 0.0;
    //vec2 texelSize = 1.0 / textureSize(engine_dirShadowMap, 0);
    //for(int x = -1; x <= 1; ++x)
    //{
    //    for(int y = -1; y <= 1; ++y)
    //    {
