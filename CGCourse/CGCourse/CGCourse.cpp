...
#endif
	KUMA::CORE_SYSTEM::App::hInstance = hInstance;
...
int main() {
	...
	auto sprite1 = KUMA::GUI::GuiHelper::CreateSprite("sprite1", std::string("Textures/btn.png"), true);
	auto btn1 = KUMA::GUI::GuiHelper::CreateButton("btn1", true);
	auto label1 = KUMA::GUI::GuiHelper::CreateLabel("label1", std::string("HELLO"), false);
	label1->setParent(sprite1);

	auto clip = KUMA::GUI::GuiHelper::CreateClip("c", 150, 55, true);
	auto scroll = KUMA::GUI::GuiHelper::CreateScroll("scroll", 250, 55, false);
	scroll->setParent(clip);
	auto layout = KUMA::GUI::GuiHelper::CreateLayout("l", KUMA::ECS::LayoutComponent::Type::HORIZONTAL, false);
	layout->setParent(scroll);
	for (int i = 0; i < 5 ; i++) {
		auto s = KUMA::GUI::GuiHelper::CreateSprite("s" + std::to_string(i), std::string("Textures/btn.png"), false);
		s->setParent(layout);
	}
