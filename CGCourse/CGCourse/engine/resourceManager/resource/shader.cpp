...
std::vector<RENDER::UniformInfo>& Shader::getUniforms() {
	return uniforms;
}
...
bool Shader::isEngineUBOMember(const std::string& uniformName) {
	return uniformName.rfind("ubo_", 0) == 0;
}

bool Shader::isEngineUniformMember(const std::string& uniformName) {
...
void Shader::setUniformVec2(const std::string& name, const MATHGL::Vector2f& vec2) {
	glUniform2f(getUniformLocation(name), vec2.x, vec2.y);
}

void Shader::setUniformVec3(const std::string& name, const MATHGL::Vector3& vec3) {
	glUniform3f(getUniformLocation(name), vec3.x, vec3.y, vec3.z);
}

void Shader::setUniformVec4(const std::string& name, const MATHGL::Vector4& vec4) {
	glUniform4f(getUniformLocation(name), vec4.x, vec4.y, vec4.z, vec4.w);
}

void Shader::setUniformMat4(const std::string& name, const MATHGL::Matrix4& mat4) {
	glUniformMatrix4fv(getUniformLocation(name), 1, GL_TRUE, mat4.data._values);
}

void Shader::setUniformMat3(const std::string& name, const MATHGL::Matrix3& mat3) {
	glUniformMatrix3fv(getUniformLocation(name), 1, GL_TRUE, mat3.data._values);
}

MATHGL::Vector2f Shader::getUniformVec2(const std::string& name) {
	GLfloat values[2];
	glGetnUniformfv(id, getUniformLocation(name), 2 * sizeof(float), values);
	return reinterpret_cast<MATHGL::Vector2f&>(values);
}

MATHGL::Vector3 Shader::getUniformVec3(const std::string& name) {
	GLfloat values[3];
	glGetnUniformfv(id, getUniformLocation(name), 3 * sizeof(float), values);
	return reinterpret_cast<MATHGL::Vector3&>(values);
}

MATHGL::Vector4 Shader::getUniformVec4(const std::string& name) {
	GLfloat values[4];
	glGetnUniformfv(id, getUniformLocation(name), 4 * sizeof(float), values);
	return reinterpret_cast<MATHGL::Vector4&>(values);
}

MATHGL::Matrix4 Shader::getUniformMat4(const std::string& name) {
	GLfloat values[16];
