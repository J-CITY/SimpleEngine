...
static std::vector<char> readFile(const std::string& filename) {
	std::ifstream file(KUMA::UTILS::getRealPath(filename), std::ios::ate | std::ios::binary);
...
void ShaderVk::createDescriptorSetLayout() {
	...
	int curset = 0;
	auto setSize = 0;
	std::vector<KUMA::RENDER::UniformInform> _mUniforms;
...
const std::unordered_map<std::string, KUMA::RENDER::UniformInform>& ShaderVk::getUniformsInfo() const {
	return mUniforms;
}
