...
//		_set(k, subname, TextureVk::create(KUMA::UTILS::getRealPath(v.get<std::string>())));
...
void MaterialDx12::onDeserialize(nlohmann::json& j) {
	...
	if (j.count("uniforms")) {
		for (auto& [k, v] : j["uniforms"].items()) {
			if (k == "u_AlbedoMap")
			{
				u_AlbedoMap = TextureDx12::Create(KUMA::UTILS::getRealPath(v.get<std::string>()));
			}
			if (k == "u_NormalMap")
			{
				u_NormalMap = TextureDx12::Create(KUMA::UTILS::getRealPath(v.get<std::string>()));
			}
