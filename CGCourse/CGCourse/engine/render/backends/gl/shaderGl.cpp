...
std::string readFile(const std::string& path) {
	std::ifstream ifs(KUMA::UTILS::getRealPath(path));
	...
	std::optional<std::string> tessEvalPath, std::optional<std::string> computePath)
{
	auto readShader = [](std::string path) {
		...
		shaderFile.open(KUMA::UTILS::getRealPath(path));
...
void ShaderGl::getReflection() {
	...
	struct UData {
		int id = 0;
		std::string name;
		KUMA::RENDER::UNIFORM_TYPE type = KUMA::RENDER::UNIFORM_TYPE::NONE;
		int arraySize = 0;
		int offset = 0;
		int blockIndex = 0;
	};
	...
	const std::unordered_map<GLenum, KUMA::RENDER::UNIFORM_TYPE> fromGlType = {
		{ GL_FLOAT_MAT4, KUMA::RENDER::UNIFORM_TYPE::MAT4 },
		{ GL_FLOAT_MAT3, KUMA::RENDER::UNIFORM_TYPE::MAT3 },
		{ GL_FLOAT_VEC4, KUMA::RENDER::UNIFORM_TYPE::VEC4 },
		{ GL_FLOAT_VEC3, KUMA::RENDER::UNIFORM_TYPE::VEC3 },
		{ GL_FLOAT_VEC2, KUMA::RENDER::UNIFORM_TYPE::VEC2 },
		{ GL_INT, KUMA::RENDER::UNIFORM_TYPE::INT },
		{ GL_FLOAT, KUMA::RENDER::UNIFORM_TYPE::FLOAT },
		{ GL_BOOL, KUMA::RENDER::UNIFORM_TYPE::BOOL },
		{ GL_SAMPLER_2D, KUMA::RENDER::UNIFORM_TYPE::SAMPLER_2D },
		{ GL_SAMPLER_3D, KUMA::RENDER::UNIFORM_TYPE::SAMPLER_3D },
		{ GL_SAMPLER_2D_ARRAY, KUMA::RENDER::UNIFORM_TYPE::SAMPLER_2D_ARRAY},
		{ GL_SAMPLER_CUBE, KUMA::RENDER::UNIFORM_TYPE::SAMPLER_CUBE }
	};
	const std::unordered_map<KUMA::RENDER::UNIFORM_TYPE, int> typeToSize = {
		{ KUMA::RENDER::UNIFORM_TYPE::MAT4, sizeof(float) * 16 },
		{ KUMA::RENDER::UNIFORM_TYPE::MAT3, sizeof(float) * 9 },
		{ KUMA::RENDER::UNIFORM_TYPE::VEC4, sizeof(float) * 4 },
		{ KUMA::RENDER::UNIFORM_TYPE::VEC3, sizeof(float) * 3 },
		{ KUMA::RENDER::UNIFORM_TYPE::VEC2, sizeof(float) * 2 },
		{ KUMA::RENDER::UNIFORM_TYPE::INT, sizeof(int) * 1 },
		{ KUMA::RENDER::UNIFORM_TYPE::FLOAT, sizeof(float) * 1 },
		{ KUMA::RENDER::UNIFORM_TYPE::BOOL, sizeof(bool) * 1 },
		{ KUMA::RENDER::UNIFORM_TYPE::SAMPLER_2D, sizeof(int) * 1 },
		{ KUMA::RENDER::UNIFORM_TYPE::SAMPLER_3D, sizeof(int) * 1 },
		{ KUMA::RENDER::UNIFORM_TYPE::SAMPLER_CUBE, sizeof(int) * 1 },
		{ KUMA::RENDER::UNIFORM_TYPE::SAMPLER_2D_ARRAY, sizeof(int) * 1 },
	};

	std::array<KUMA::RENDER::SHADER_TYPE, 6> shaderTypes = {
		KUMA::RENDER::SHADER_TYPE::VERTEX,
		KUMA::RENDER::SHADER_TYPE::TESSELLATION_CONTROL,
		KUMA::RENDER::SHADER_TYPE::TESSELLATION_EVALUATION,
		KUMA::RENDER::SHADER_TYPE::GEOMETRY,
		KUMA::RENDER::SHADER_TYPE::FRAGMENT,
		KUMA::RENDER::SHADER_TYPE::COMPUTE
	};
	...
	for (int unif = 0; unif < numActiveUniforms; ++unif) {
		...
		KUMA::RENDER::UniformInform uniform;
		uniform.name = data.back().name;
		uniform.type = KUMA::RENDER::UniformInform::TYPE::UNIFORM;
	...
	for (int blockIx = 0; blockIx < numUniformBlocks; ++blockIx) {
		...
		uniform.type = KUMA::RENDER::UniformInform::TYPE::UNIFORM_BUFFER;
		...
		for (auto i : uniformIxs) {
			uniform.members.push_back({
				...
				data[i].offset, typeToSize.at(data[i].type), data[i].arraySize});
		}
...
void ShaderGl::setMat4(const std::string& name, const KUMA::MATHGL::Matrix4& mat) const
{
	glUniformMatrix4fv(glGetUniformLocation(ID, name.c_str()), 1, GL_TRUE, mat.data._values);
}


void ShaderGl::setVec2(const std::string& name, const KUMA::MATHGL::Vector2f& vec2) {
	glUniform2f(getUniformLocation(name), vec2.x, vec2.y);
}

void ShaderGl::setVec3(const std::string& name, const KUMA::MATHGL::Vector3& vec3) {
	glUniform3f(getUniformLocation(name), vec3.x, vec3.y, vec3.z);
}

void ShaderGl::setVec4(const std::string& name, const KUMA::MATHGL::Vector4& vec4) {
	glUniform4f(getUniformLocation(name), vec4.x, vec4.y, vec4.z, vec4.w);
}

void ShaderGl::setMat4(const std::string& name, const KUMA::MATHGL::Matrix4& mat4) {
	glUniformMatrix4fv(getUniformLocation(name), 1, GL_TRUE, mat4.data._values);
}

void ShaderGl::setMat3(const std::string& name, const KUMA::MATHGL::Matrix3& mat3) {
...
KUMA::MATHGL::Vector2f ShaderGl::getUniformVec2(const std::string& name) {
	...
	return reinterpret_cast<KUMA::MATHGL::Vector2f&>(values);
}
...
KUMA::MATHGL::Vector3 ShaderGl::getUniformVec3(const std::string& name) {
	...
	return reinterpret_cast<KUMA::MATHGL::Vector3&>(values);
}
...
KUMA::MATHGL::Vector4 ShaderGl::getUniformVec4(const std::string& name) {
	...
	return reinterpret_cast<KUMA::MATHGL::Vector4&>(values);
}
...
KUMA::MATHGL::Matrix4 ShaderGl::getUniformMat4(const std::string& name) {
	...
	return reinterpret_cast<KUMA::MATHGL::Matrix4&>(values);
}
