...
std::optional<KUMA::Ref<KUMA::ECS::CameraComponent>> mainCameraComponentDx = std::nullopt;
...
GameRendererDx12::GameRendererDx12(HINSTANCE hInstance, KUMA::CORE_SYSTEM::Core& context): mContext(context), mhAppInst(hInstance){
	assert(mApp == nullptr);
	mApp = this;
...
void GameRendererDx12::renderScene(KUMA::Ref<KUMA::ECS::CameraComponent> mainCameraComponent) {
	auto [winWidth, winHeight] = mContext.window->getSize();
...
bool GameRendererDx12::Initialize() {
	...
	mEmptyTexture = TextureDx12::Create(KUMA::UTILS::getRealPath("Textures/snow.png"));
