...
namespace std {
	...
	template <typename T> struct hash<KUMA::object_ptr<T>> {
		constexpr size_t operator()(KUMA::object_ptr<T> const& p) const
			noexcept {
			return hash<T*>()(p.get());
		}
	};
	...
	typename std::enable_if<
		...
		KUMA::object_ptr<To>>::type
		static_pointer_cast(KUMA::object_ptr<From> p) {
	...
	typename std::enable_if<
		...
		KUMA::object_ptr<To>>::type
		dynamic_pointer_cast(KUMA::object_ptr<From> p) {
