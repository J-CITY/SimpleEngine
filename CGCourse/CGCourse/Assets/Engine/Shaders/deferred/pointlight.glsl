    ...
    out vec3 specular, out vec3 diffuse)
{
    ...
    //specular = vec3(0.0, 1.0f, 0.0f);
    //diffuse = vec3(0.0, 1.0f, 0.0f);
    //return;
...
vec3 CalcPointLight(vec2 TexCoord, PointLight light, vec3 normal, vec3 fragPos, vec3 viewDir, samplerCube map_shadow)
{
    ...
    float shadowFactor = 1.0f;
    //shadowFactor = CalcShadowFactor3D(lightDir, viewDir, light.radius, 0.15f, map_shadow);
    ...
    {
        float closestDepth = texture(map_shadow, fragToLight + gridSamplingDisk[i] * diskRadius).r;
    ...
    //float closestDepth = texture(map_shadow, lightDir).r;
    //return vec3(closestDepth / light.radius);
    ...
    float attenuation = 1.0 / (1.0f + 0.5f * distance + 0.25f * (distance * distance));
 
    // Комбинируем результаты
    vec3 ambient =  vec3(1.0f, 0.0f, 0.0f) * vec3(texture(albedoTex, TexCoord));
    vec3 diffuse =  vec3(light.color) * diff * vec3(texture(albedoTex, TexCoord));
    vec3 specular = vec3(light.color) * spec * vec3(texture(materialTex, TexCoord));
    ambient *= attenuation;
    diffuse *= attenuation;
...
void main()
{
    ...
    //vec3 totalColor = CalcPointLight(TexCoord, light, fragment.normal, fragment.position, viewDirection, lightDepthMap);
    ...
    //OutColor = vec4(pointLight.color.xyz, 1.0f);
