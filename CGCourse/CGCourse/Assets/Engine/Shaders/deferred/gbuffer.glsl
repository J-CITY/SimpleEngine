...
void main()
{
    ...
    vec4 modelPos = parentModel * vec4(position.xyz, 1.0);
    
    mat3 _normalMatrix = transpose(inverse(mat3(model)));
    mat3 normalSpaceMatrix = _normalMatrix;

    vec3 T = normalize(normalSpaceMatrix * tangent);
    //vec3 B = normalize(normalSpaceMatrix * bitangent);
    vec3 N = normalize(normalSpaceMatrix * normal);
    T = normalize(T - dot(T, N) * N);
    vec3 B = cross(N, T);

    vsout.TBN = transpose(mat3(T, B, N));
    vsout.Normal = N;
...
void main()
{
    ...
    //OutAlbedo = vec4(texture(map_albedo, TexCoord).rgb, 1.0f);
    OutNormal = vec4(0.5f * normal + 0.5f, 1.0f);
    //OutNormal = vec4(texture(map_normal, TexCoord).rgb, 1.0f);
